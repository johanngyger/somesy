...
def test_linkedin_to_slack_new_posts(mock_linkedin_posts, mock_slack_messages):
    ...
    with patch("sync.linkedin.recent_linkedin_posts", return_value=mock_linkedin_posts) as mock_linkedin:
        with patch("sync.slackc.slack_messages", return_value=mock_slack_messages) as mock_slack_get:
...
def test_linkedin_to_slack_no_new_posts():
    ...
    with patch("sync.linkedin.recent_linkedin_posts", return_value=[{"id": "post-1"}]) as mock_linkedin:
